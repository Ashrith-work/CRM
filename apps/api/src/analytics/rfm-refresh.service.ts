...
export class RfmRefreshService {
  ...
  async refreshView(): Promise<void> {
    try {
      await this.prisma.$executeRawUnsafe('REFRESH MATERIALIZED VIEW CONCURRENTLY customer_rfm');
    } catch (err) {
      this.logger.warn(`CONCURRENTLY refresh failed (${(err as Error).message}); falling back`);
      await this.prisma.$executeRawUnsafe('REFRESH MATERIALIZED VIEW customer_rfm');
    }
  }

  /** Refresh the view, then rewrite features for every org present in it. */
  async refreshAll(now = new Date()): Promise<{ orgs: number; customers: number }> {
    ...
    this.logger.log(`RFM refresh: ${customers} customer(s) across ${orgs.length} org(s)`);
